...
static void usage(const char *prog) {
    fprintf(
        ...
        prog, TASK_COMM_LEN, TASK_COMM_LEN - 1, MAX_PREFIXES, MAX_COMMS, prog,
        prog);
}
...
int main(int argc, char **argv) {
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (!strcmp(argv[i], "--comm") && i + 1 < argc) {
            ...
            if (current_comm_idx < 0) {
                fprintf(stderr,
                        "error: too many distinct --comm values "
                        "(max %d)\n",
                        MAX_COMMS);
                return 1;
            }
        ...
        if (!strcmp(argv[i], "--allow") && i + 1 < argc) {
            ...
            const char *p = argv[++i];
            size_t plen =
                canonicalise_prefix(p, acc.allows[acc.num_allows].path);
            if (plen == 0) {
    ...
    // push every (comm, prefix) entry into the LPM trie
    struct lpm_key key;
